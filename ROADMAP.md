...
- [x] **오늘의 한 줄** (retro)
- [x] **모바일 축약 뷰** — `@media (max-width: 768px)` 업무명+완료 2컬럼
...
- [x] 중요 업무 카드 그리드 UI (`state.importantTasks`, `state.routines`)
- [x] 카드 CRUD + 우선순위/누적시간 표시
- [x] **중요 업무 → 오늘 카드 드롭** — 우클릭 메뉴 + BroadcastChannel 알림
- [x] **자동 링크** — 업무명 매칭 시 ★ 표시 + 누적 시간 집계(이번 주+히스토리)
- [x] **미완 업무 이월** — 우클릭 "다음 영업일로 이월" (금→월 자동 큐)
- [x] **자동 백업 알림** — 마지막 export 7일 초과 시 상단 뱃지

### Phase R3 — history.html 신규
- [x] 주간 테이블 (요일별 업무 목록, 완료율)
- [x] `state.history` 기반 과거 주 탐색
- [x] **주간 자동 요약** — 금요일 18시 이후 자동 생성 + 수동 ↻/복사 버튼
- [x] **24시간 타임라인 바** — SVG, 프로젝트별 색상 블록
- [x] **월간 히트맵** — 8주 완료 수 기준 + hover 툴팁
- [x] **프로젝트 도넛** — SVG, 이번 주/이번 달 토글
